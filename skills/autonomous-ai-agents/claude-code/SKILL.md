...
## ⚠️ Auth Mode Matters: Three Paths

**This is the #1 gotcha with Claude Code v2.** There are three auth paths:

| Path | How | Works with `-p`? | Works with TUI? | Notes |
|---|---|---|---|---|
| **API Key** | `export ANTHROPIC_API_KEY="sk-..."` | ✅ Yes | ✅ Yes | Get from console.anthropic.com/settings/keys |
| **OAuth** (browser login) | `claude auth login` | ❌ "Invalid API key" | ✅ Yes | Credentials in `~/.claude/.credentials.json` |
| **SDK + OAuth** (BEST for agents) | Use `claude-agent-sdk` Python lib, **unset** `ANTHROPIC_API_KEY` | N/A — SDK is programmatic | N/A | OAuth works when env var is cleared; 401 when var is set to OAuth token |

**Recommended for Hermes agents: SDK + OAuth path.**

```python
# claude_agent_sdk is pre-installed at /usr/local/lib/python3.13/site-packages
from claude_agent_sdk import ClaudeAgentOptions, ClaudeSDKClient
import asyncio

async def delegate(task):
    # CRITICAL: Remove ANTHROPIC_API_KEY from env so SDK uses OAuth from ~/.claude/.credentials.json
    import os
    env = os.environ.copy()
    ...
    options = ClaudeAgentOptions(
        model="sonnet",                    # Always use sonnet per user preference
...
**Gotcha:** If `ANTHROPIC_API_KEY` is set in the inherited environment (e.g., from a parent shell that ran `claude auth login`), it will be set to the OAuth **access token** which is NOT a valid API key. The SDK then fails with HTTP 401. **Always `unset ANTHROPIC_API_KEY` before using the SDK with OAuth.**

**Cross-user issue:** Credentials are stored in `$HOME/.claude/.credentials.json`. If the agent runs as a different user than the one who logged in, copy the credentials file and `chown` it.

**User preference (explicit):** Always execute Claude Code with `--model sonnet --dangerously-skip-permissions`. In SDK terms: `model="sonnet"`, `permission_mode="bypassPermissions"`. Delegate all logic-heavy coding tasks to Claude Code — the agent should only do simple one-liner edits directly.
...
## Hermes SDK Integration (PREFERRED)

The `claude-agent-sdk` Python package provides direct programmatic access to Claude Code.
This is the **preferred** integration method for Hermes agents — no PTY, no tmux, no
remote-control needed. It uses your existing OAuth credentials automatically.

### Prerequisites

- `claude-agent-sdk` Python package (check: `pip show claude-agent-sdk`)
- OAuth credentials in `~/.claude/.credentials.json` (from `claude auth login`)
- **CRITICAL**: The `ANTHROPIC_API_KEY` env var must NOT be set — it overrides OAuth.
  The OAuth token in the credentials file is NOT a valid Anthropic API key.
  Unset it: `unset ANTHROPIC_API_KEY`

### Quick Test

```bash
unset ANTHROPIC_API_KEY && export HOME=/data && cd /project && python3 -c "
from claude_agent_sdk import ClaudeAgentOptions, ClaudeSDKClient
import asyncio
...
async def main():
    ...
    async with ClaudeSDKClient(options=options) as client:
        await client.query('Reply with exactly: SDK_READY')
        async for msg in client.receive_response():
...
If this prints `SDK_READY`, the integration works.

### Persistent Wrapper (Recommended)

Use the wrapper script at `/data/workspace/perp-trading-bot/scripts/claude_delegate.py`:

```bash
cd /data/workspace/perp-trading-bot
python3 scripts/claude_delegate.py "Add error handling to src/risk_engine.py"
python3 scripts/claude_delegate.py --file /tmp/task.txt
echo "Task description" | python3 scripts/claude_delegate.py --stdin
python3 scripts/claude_delegate.py "Refactor" --model sonnet
```

The wrapper handles environment cleanup (unsetting ANTHROPIC_API_KEY, setting HOME, etc.).

### For Delegation via delegate_task

When spawning a coding subagent with `delegate_task`, pass the SDK-based wrapper:

```
goal: "Delegate this coding task to Claude Code using the SDK wrapper"
context: |
  Run this command to complete the task:
  cd /data/workspace/perp-trading-bot && python3 scripts/claude_delegate.py "THE TASK"
  
  The wrapper uses Claude Code v2 via the claude-agent-sdk with:
  - model: sonnet
  - permission_mode: bypassPermissions
  - tools: Read, Write, Edit, Bash, Glob, Grep, Task
  - auth: OAuth from ~/.claude/.credentials.json
```
...
| Method | Why Not |
|--------|---------|
| `claude -p "task"` | Requires ANTHROPIC_API_KEY (OAuth tokens don't work) |
| `claude remote-control` | Requires interactive TUI workspace trust — can't automate in Docker |
| tmux interactive | tmux server won't persist in Docker containers |
| PTY + stdin pipe | Claude Code detects pipe and falls back to print mode |

### Always Use

- Model: `sonnet` (as specified by user)
- Permission mode: `bypassPermissions` (maps to `--dangerously-skip-permissions`)
- Clear `ANTHROPIC_API_KEY` from environment before launching
...
13. **Auth status may differ between sessions** — `claude auth status` can show `loggedIn: false` from the agent's process even when the user authenticated from their shell. Credentials may be stored in a different home directory or env context. See `references/claude-code-auth-troubleshooting.md` for diagnosis steps.
14. **Tmux server won't persist in Docker** — `tmux start-server` and `tmux new-session -d` fail silently in Docker containers because the server daemonizes and loses its parent process. Use the `claude-agent-sdk` Python library instead for programmatic delegation — it handles PTY creation internally and works reliably in containers.
15. **`claude remote-control` requires workspace trust** — The remote-control server checks for workspace trust before starting. Trust is a TUI-only interaction that cannot be automated from a non-interactive process in Docker. Use the `claude-agent-sdk` instead, which bypasses the trust check entirely.
16. **SDK + OAuth is the recommended agent path** — The `claude-agent-sdk` (v0.2.87, pre-installed at `/usr/local/lib/python3.13/site-packages`) provides `ClaudeSDKClient` with `query()` and `receive_response()`. It works with OAuth credentials when `ANTHROPIC_API_KEY` is unset. This is more reliable than tmux orchestration or print mode for agent-to-agent delegation.

## Rules for Hermes Agents

1. **Use the claude-agent-sdk Python library for delegation** — More reliable than tmux or print mode. Pre-installed at `/usr/local/lib/python3.13/site-packages`.
2. **Always unset `ANTHROPIC_API_KEY` before using the SDK** — Required for OAuth to work; see `references/claude-agent-sdk-delegation.md`.
3. **Prefer print mode (`-p`) only when API key is explicitly set** — Otherwise use SDK.
4. **Use tmux for multi-turn interactive work** — but note: tmux does not persist in Docker containers (pitfall #14).
...
7. **Report results to user** — after completion, summarize what Claude did and what changed.
8. **Don't kill slow sessions** — Claude may be doing multi-step work; check progress instead.
9. **Use `--allowedTools`** — restrict capabilities to what the task actually needs.

## References

- `references/claude-code-auth-troubleshooting.md` — Auth diagnosis, cross-user setup
- `references/claude-agent-sdk-delegation.md` — Python SDK delegation pattern (recommended approach)

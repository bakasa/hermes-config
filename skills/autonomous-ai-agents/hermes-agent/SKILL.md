...
### Profiles & Multi-Instance Cloning

Detailed clone semantics, Docker gateway constraints, auth exhaustion after cloning, and the wrapper PATH fix: see `references/profile-cloning-and-docker-gateway.md`.
...
**Docker constraint:** one container = one gateway process. Run a second profile in a new container, not inside the existing one.
